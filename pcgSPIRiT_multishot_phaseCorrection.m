% phase error-corrected multi-shot EPI SPIRiT reconstruction
%
% Ziyu Li, Wenchuan Wu, University of Oxford, 2024

function [res,RESVEC] = pcgSPIRiT_multishot_phaseCorrection(kdata, phs, sampling_order, GOP, nIter, sz, nch,z_start,lambda,alpha)

y = epi2D_phs_multishot_fw_adj(kdata,phs,sampling_order,z_start);
y = fft2c(y);
...
res = reshape(tmpres(1:N),[sz nch]);
    ...
    function y = afun(x,phs,sampling_order,GOP,sz,nch,z_start,lambda,alpha)
        ...
        x_im = ifft2c(x_k_in);
        x_k_ms = epi2D_phs_multishot_fw(x_im, phs, sampling_order,z_start); % DP
        x_im = epi2D_phs_multishot_fw_adj(x_k_ms,phs,sampling_order,z_start); % (DP)^H
        x_k = fft2c(x_im);
