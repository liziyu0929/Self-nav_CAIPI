...
% z = zeros([sx sy nCoils]); % Tikhonov regularization term
% yy = fft2c(ESP*(ESP'*(ifft2c(y)))); yy = [-yy(:); z(:)];

[tmpres,FLAG,RELRES,ITER,RESVEC] = lsqr(@aprod,yy,1e-6,nIter, speye(N,N),speye(N,N),x0(idx_nacq),ESP,sx,sy,nCoils,idx_nacq, lambda);

% [tmpres,FLAG,RELRES,ITER,RESVEC] = lsqr(@aprod,yy,1e-6,nIter, speye(N,N),speye(N,N),x0(:),ESP,sx,sy,nCoils,lambda);

res = y;
res(idx_nacq) = tmpres;
imgs = ESP'*(ifft2c(res));
% res = reshape(tmpres,[sx sy nCoils]);
% imgs = ESP'*(ifft2c(res));
...
% function [res,tflag] = aprod(x,ESP,sx,sy,nCoils,lambda,tflag)
% 	
% 	
% 	if strcmp(tflag,'transp');
% 		tmpy = reshape(x(1:sx*sy*nCoils),sx,sy,nCoils);
...
%         res = ESP*(ESP'*tmpy);
%         res = fft2c(res);
%         res = res(:)+ x(sx*sy*nCoils+1:end)*lambda;
% 	
%     else
% 		tmpx = reshape(x, [sx,sy,nCoils]);
%         tmpx = ifft2c(tmpx);
% 		res = ESP*(ESP'*tmpx);
%         res = fft2c(res);
% 		res = [res(:) ; lambda*x(:)];
